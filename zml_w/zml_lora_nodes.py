...
async def view_lora_preview(request):
    ...
    # 优先在 zml 子目录中查找预览图（zml > 同层级）
    target_path_zml = os.path.join(actual_lora_dir, "zml", preview_filename_with_ext)
    if os.path.isfile(target_path_zml):
        return web.FileResponse(target_path_zml, headers={"Content-Disposition": f"filename=\"{os.path.basename(target_path_zml)}\""})

    # 若 zml 不存在，则回退至与LoRA同层级的预览图
    target_path_same_level = os.path.join(actual_lora_dir, preview_filename_with_ext)
    if os.path.isfile(target_path_same_level):
        return web.FileResponse(target_path_same_level, headers={"Content-Disposition": f"filename=\"{os.path.basename(target_path_same_level)}\""})

    return web.Response(status=404, text=f"预览图未找到: {target_path_same_level} 或 {target_path_zml}")
...
async def save_preview(request):
    ...
    # 增加目标保存位置开关：默认同层级，支持写入 zml 子目录
    target = str(body.get("target", "same")).lower()
    target = "zml" if target == "zml" else "same"

    if target == "zml":
        # 保存到 zml 子目录：<loras_root>/<subdir>/zml/<mylora>.<ext>
        lora_dir_relative_path = os.path.dirname(lora_relative_path)
        destination_dir = os.path.join(lora_root_dir, lora_dir_relative_path, "zml")
        destination_path = os.path.join(destination_dir, f"{os.path.basename(lora_path_no_ext)}{source_ext}")
    else:
        # 同层级保存：<loras_root>/<subdir>/<mylora>.<ext>
    ...
    # 前端统一返回不含 zml 的相对路径，以适配查看路由的 zml 优先逻辑
...
async def get_images(request):
    ...
    for lora_filename in lora_files: # lora_filename is like "subdir/mylora.safetensors"
        ...
        # 先在 zml 子目录查找预览图，若无则查找同层级（zml > 同层级）
        for ext in [".png", ".jpg", ".jpeg", ".webp"]:
            preview_path_zml = os.path.join(zml_dir, f"{lora_basename_no_ext}{ext}")
            if os.path.isfile(preview_path_zml):
                lora_dir_relative = os.path.dirname(lora_filename) # e.g. "subdir"
                preview_basename = os.path.basename(preview_path_zml) # e.g. "mylora.png"
...
async def get_lora_file(request):
    ...
    try:
        ...
        if file_type == "txt":
            ...
            file_ext = ".json"
        # 先尝试 zml 子目录读取，若不存在则尝试同层级（zml > 同层级）
        file_path_zml = os.path.join(lora_dir, "zml", f"{lora_basename_no_ext}{file_ext}")
        file_path_same = os.path.join(lora_dir, f"{lora_basename_no_ext}{file_ext}")

        target_file = None
        if os.path.exists(file_path_zml):
            target_file = file_path_zml
...
async def save_lora_file(request):
    ...
    try:
        ...
        if file_type == "txt":
            ...
            file_ext = ".json"
        if target == "zml":
            file_path = os.path.join(lora_dir, "zml", f"{lora_basename_no_ext}{file_ext}")
        else:
...
@server.PromptServer.instance.routes.post(ZML_API_PREFIX + "/probe_save_targets")
async def probe_save_targets(request):
    """
    探测指定LoRA的保存位置可用性：
    - zml子目录是否存在
    - zml子目录下是否存在可读取文件（txt/json/log）
    - 同层级是否存在可读取文件（txt/json/log）

    请求体: {"lora_filename": "relative/path/to/lora.safetensors"}
    响应: {
      "status": "success",
      "zml_dir_exists": true/false,
      "zml_files": {"txt": true/false, "json": true/false, "log": true/false},
      "same_files": {"txt": true/false, "json": true/false, "log": true/false},
      "zml_has_readable": true/false,
      "same_has_readable": true/false
    }
    """
    try:
        body = await request.json()
        ...
        zml_dir = os.path.join(lora_dir, "zml")
        zml_dir_exists = os.path.isdir(zml_dir)

        def check_file(path):
            return os.path.exists(path) and os.access(path, os.R_OK)

        same_txt = os.path.join(lora_dir, f"{lora_basename_no_ext}.txt")
        same_json = os.path.join(lora_dir, f"{lora_basename_no_ext}.json")
        same_log = os.path.join(lora_dir, f"{lora_basename_no_ext}.log")

        zml_txt = os.path.join(zml_dir, f"{lora_basename_no_ext}.txt")
        zml_json = os.path.join(zml_dir, f"{lora_basename_no_ext}.json")
        zml_log = os.path.join(zml_dir, f"{lora_basename_no_ext}.log")
        ...
        zml_files = {
            "txt": check_file(zml_txt),
            "json": check_file(zml_json),
            "log": check_file(zml_log),
        }

        result = {
            "status": "success",
            "zml_dir_exists": zml_dir_exists,
            "zml_files": zml_files,
            "same_files": same_files,
            "zml_has_readable": any(zml_files.values()),
        ...
        print(f"[ZML_Parser] 探测保存位置时出错: {e}")
        return web.json_response({"status": "error", "message": f"服务器内部错误: {e}"}, status=500)
...
async def fetch_civitai_metadata_api(request): # 重命名函数以避免与内部辅助函数fetch_civitai_data_by_hash混淆
    ...
    try:
        ...
        lora_dir = os.path.dirname(lora_full_path)
        # 目标目录（根据 target 选择同层级或 zml 子目录）
        if target == "zml":
            save_dir = os.path.join(lora_dir, "zml")
        else:
            save_dir = lora_dir
        lora_basename_no_ext = os.path.splitext(os.path.basename(lora_relative_filename))[0]
        ...
        if civitai_data:
            ...
            if civitai_data.get('images'):
                ...
                img_dest_path = os.path.join(save_dir, f"{lora_basename_no_ext}{img_ext}")
            ...
            if civitai_data.get('trainedWords'):
                ...
                os.makedirs(save_dir, exist_ok=True)
                txt_dest_path = os.path.join(save_dir, f"{lora_basename_no_ext}.txt")
                try:
            ...
            if preferred_weight is not None:
                os.makedirs(save_dir, exist_ok=True)
                log_dest_path = os.path.join(save_dir, f"{lora_basename_no_ext}.log")
            ...
            os.makedirs(save_dir, exist_ok=True)
            json_dest_path = os.path.join(save_dir, f"{lora_basename_no_ext}.json")
            try:
...
async def delete_lora_file(request):
    ...
    try:
        ...
        # 删除lora根目录下的相关文件（同层级）
        pattern_same = os.path.join(lora_dir, f"{lora_basename_no_ext}.*")
        related_files_same = glob.glob(pattern_same)

        for related_file in related_files_same:
            # 跳过主LoRA文件（已经删除）
        ...
        # 同步删除 zml 子目录中的同名衍生文件
        zml_dir = os.path.join(lora_dir, "zml")
        if os.path.isdir(zml_dir):
            pattern_zml = os.path.join(zml_dir, f"{lora_basename_no_ext}.*")
            related_files_zml = glob.glob(pattern_zml)
            for related_file in related_files_zml:
                try:
                    os.remove(related_file)
                    deleted_files.append(os.path.join("zml", os.path.basename(related_file)))
                except Exception as e:
                    print(f"[ZML_Parser] 删除 zml 子目录文件时出错 {related_file}: {e}")
...
class ZmlLoraMetadataParser:
    ...
    def INPUT_TYPES(s):
        return {
            ...
            "optional": {
                ...
                "保存默认权重为log": ("BOOLEAN", {"default": False}),
                "保存位置": (["同层级", "zml子目录"],),
            }
        }
    ...
    def parse_and_save_metadata(self, lora_名称, 保存首张图像=False, 保存触发词为txt=False, 保存介绍为json=False, 保存默认权重为log=False, 保存位置="同层级"):
        ...
        target = "zml" if str(保存位置).lower() == "zml子目录" else "same"
        save_dir = os.path.join(lora_dir, "zml") if target == "zml" else lora_dir
        
        parsed_info_str = f"LoRA: {lora_名称}\n"
        parsed_info_str += f"保存位置: {'zml子目录' if target == 'zml' else '同层级'}\n"
        
        if 保存首张图像 or 保存触发词为txt or 保存介绍为json or 保存默认权重为log:
            ...
            if civitai_data:
                ...
                if 保存默认权重为log:
                    preferred_weight = extract_lora_weight_from_civitai_data(civitai_data, lora_名称)
                    if preferred_weight is not None:
                        os.makedirs(save_dir, exist_ok=True)
                        log_dest_path = os.path.join(save_dir, f"{lora_basename_no_ext}.log")
                        try:
                            create_or_update_log_file(log_dest_path, preferred_weight)
                            parsed_info_str += f"默认权重(log): {preferred_weight}\n"
                            print(f"[ZML_Parser] 默认权重已保存: {log_dest_path}")
                        except Exception as e:
                            parsed_info_str += "默认权重保存失败。\n"
                            print(f"[ZML_Parser] 保存默认权重到.log失败: {e}")
                    else:
                        parsed_info_str += "未从Civitai中匹配到默认权重。\n"
        ...
        # 读取预览图：优先 zml 子目录，其次同层级
        for ext in ['.png', '.jpg', '.jpeg', '.webp']:
            preview_path_zml = os.path.join(lora_dir, "zml", f"{lora_basename_no_ext}{ext}")
            preview_path_same = os.path.join(lora_dir, f"{lora_basename_no_ext}{ext}")
            candidate_path = preview_path_zml if os.path.isfile(preview_path_zml) else preview_path_same
            if os.path.isfile(candidate_path):
                try:
                    ...
                    print(f"[ZML_Parser] 读取预览图时出错 {candidate_path}: {e}")
        # 读取 txt：优先 zml 子目录，其次同层级
        txt_filepath_zml = os.path.join(lora_dir, "zml", f"{lora_basename_no_ext}.txt")
        txt_filepath_same = os.path.join(lora_dir, f"{lora_basename_no_ext}.txt")
        txt_filepath = txt_filepath_zml if os.path.isfile(txt_filepath_zml) else txt_filepath_same
        if os.path.isfile(txt_filepath):
            with open(txt_filepath, 'r', encoding='utf-8') as f:
                txt_content = f.read()
        # 读取 json：优先 zml 子目录，其次同层级
        json_filepath_zml = os.path.join(lora_dir, "zml", f"{lora_basename_no_ext}.json")
        json_filepath_same = os.path.join(lora_dir, f"{lora_basename_no_ext}.json")
        json_filepath = json_filepath_zml if os.path.isfile(json_filepath_zml) else json_filepath_same
