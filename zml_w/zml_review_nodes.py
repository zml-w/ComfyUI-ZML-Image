...
# ==========================================================================================
# 节点 1: ZML_自动打码 (原节点，保持不变)
# ==========================================================================================
class ZML_AutoCensorNode:
    """
    ZML_自动打码节点：
    通过YOLO模型自动检测物体并进行覆盖处理。
    """
    @classmethod
    def INPUT_TYPES(cls):
    ...
    def apply_overlay(self, source_cv2, mask_cv, mode, overlay_image_tensor, mosaic_count):
        ...
        if mode == "马赛克":
            ...
            if resized_overlay.shape[2] == 4:
                overlay_rgb = resized_overlay[:, :, :3]; alpha_mask = resized_overlay[:, :, 3]
                final_alpha_mask = cv2.bitwise_and(mask_roi, alpha_mask)
                alpha_float = final_alpha_mask.astype(float) / 255.0; alpha_float_3d = np.stack([alpha_float]*3, axis=2)
                blended_roi = (overlay_rgb * alpha_float_3d) + (roi * (1 - alpha_float_3d))
                np.copyto(source_cv2[y:y+h, x:x+w], blended_roi.astype(np.uint8), where=np.stack([mask_roi]*3, axis=2).astype(bool))
            else:
                np.copyto(source_cv2[y:y+h, x:x+w], resized_overlay, where=np.stack([mask_roi]*3, axis=2).astype(bool))
        return source_cv2
...
# 节点 2: ZML_自定义打码 (新增节点)
# ==========================================================================================
class ZML_CustomCensorNode(ZML_AutoCensorNode): # 继承ZML_AutoCensorNode以复用所有辅助函数
    """
    ZML_自定义打码节点：
    接收外部传入的遮罩(MASK)，并进行与自动打码节点相同的覆盖处理。
    """
    ...
    def process(self, 原始图像, 遮罩, 覆盖模式, 马赛克数量, 遮罩缩放系数, 遮罩膨胀, 覆盖图=None):
        ...
        # 将输入的MASK张量转换为OpenCV格式的蒙版
        # MASK是单通道的，所以我们直接squeeze(0)
...
NODE_CLASS_MAPPINGS = {
    ...
    "ZML_CustomCensorNode": ZML_CustomCensorNode, # 新增节点的映射
}
NODE_DISPLAY_NAME_MAPPINGS = {
    "ZML_AutoCensorNode": "ZML_自动打码",
    "ZML_CustomCensorNode": "ZML_自定义打码", # 新增节点的显示名称
}
