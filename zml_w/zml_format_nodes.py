...
class ZML_SelectTextV3:
    ...
    def INPUT_TYPES(cls):
        return {
            ...
            "optional": {
                "可选输入": ("STRING", {"forceInput": True}), # 新增的可选外部文本输入接口
            },
            "hidden": {
    ...
    def execute(self, separator, selectTextV3_data, unique_id=None, extra_pnginfo=None, 可选输入=None):
        ...
        # 添加可选的外部文本输入（如果有），放在最前面
        if 可选输入 is not None and 可选输入.strip():
            final_parts.append(可选输入)
        ...
        if random_enabled and enabled_entries:
            ...
            try:
                ...
                final_parts.extend(selected_entries)
            except (ValueError, TypeError):
                # 如果转换失败，使用所有启用的条目
                final_parts.extend(enabled_entries)
        else:
            # 不启用随机时，使用所有启用的文本条目
            final_parts.extend(enabled_entries)
