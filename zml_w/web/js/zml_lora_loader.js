...
import { $el } from "/scripts/ui.js"; // 确保这里引入的是 ComfyUI 的 $el
...
app.registerExtension({
	...
	async beforeRegisterNodeDef(nodeType, nodeData) {
        ...
        if (nodeData.name === "ZmlPowerLoraLoader") {
            ...
            function createLoraContentEditModal() {
                ...
                const showSaveTargetDialog = () => new Promise((resolve) => {
                    const overlay = zmlCreateEl("div", {
                        style: `position: fixed; inset: 0; background: rgba(0,0,0,0.1); z-index: 20201;`
                    });
                    const dialog = zmlCreateEl("div", {
                        style: `position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); background: #1a1a1a; border: 1px solid #4a4a4a; border-radius: 8px; padding: 16px; width: 420px; z-index: 20202; color: #f0f0f0; box-shadow: 0 4px 16px rgba(0,0,0,0.8);`
                    });
                    const title = zmlCreateEl("p", { style: `margin: 0 0 12px; color: #fff; font-weight: bold;` }, "选择保存位置");
                    const tips = zmlCreateEl("p", { style: `margin: 0 0 16px; color: #ccc; line-height: 1.5;` }, "请选择要将修改内容保存到哪里。您也可以选择同时保存到两个位置。");
                    const targetContainer = zmlCreateEl("div", { style: `margin: 8px 0 16px; display: flex; gap: 16px; align-items: center; flex-wrap: wrap;` });
                    const targetLabel = zmlCreateEl("span", { style: `color: #ccc; font-size: 13px;` }, "保存位置");
                    const sameLabel = zmlCreateEl("label", { style: `display: flex; gap: 6px; align-items: center; cursor: pointer;` });
                    const inputSame = document.createElement("input"); inputSame.type = "radio"; inputSame.name = "zml-edit-save-target"; inputSame.value = "same"; inputSame.checked = true;
                    const sameText = zmlCreateEl("span", {}, "同层级"); sameLabel.appendChild(inputSame); sameLabel.appendChild(sameText);
                    const zmlLabel = zmlCreateEl("label", { style: `display: flex; gap: 6px; align-items: center; cursor: pointer;` });
                    const inputZml = document.createElement("input"); inputZml.type = "radio"; inputZml.name = "zml-edit-save-target"; inputZml.value = "zml";
                    const zmlText = zmlCreateEl("span", {}, "zml子目录"); zmlLabel.appendChild(inputZml); zmlLabel.appendChild(zmlText);
                    const bothLabel = zmlCreateEl("label", { style: `display: flex; gap: 6px; align-items: center; cursor: pointer;` });
                    const inputBoth = document.createElement("input"); inputBoth.type = "radio"; inputBoth.name = "zml-edit-save-target"; inputBoth.value = "both";
                    const bothText = zmlCreateEl("span", {}, "同时保存"); bothLabel.appendChild(inputBoth); bothLabel.appendChild(bothText);
                    targetContainer.appendChild(targetLabel);
                    targetContainer.appendChild(sameLabel);
                    targetContainer.appendChild(zmlLabel);
                    targetContainer.appendChild(bothLabel);

                    const buttons = zmlCreateEl("div", { style: `display: flex; gap: 12px; justify-content: flex-end;` });
                    const okBtn = zmlCreateEl("button", { style: `padding: 8px 16px; background: #4CAF50; border: none; color: #fff; border-radius: 4px; cursor: pointer;` }, "确定");
                    const cancelBtn2 = zmlCreateEl("button", { style: `padding: 8px 16px; background: #666; border: none; color: #fff; border-radius: 4px; cursor: pointer;` }, "取消");
                    const close = () => { try { document.body.removeChild(overlay); document.body.removeChild(dialog); } catch (_) {} };
                    okBtn.onclick = () => {
                        const val = inputBoth.checked ? "both" : (inputZml.checked ? "zml" : "same");
                        close();
                        resolve(val);
                    };
                    cancelBtn2.onclick = () => { close(); resolve(null); };
                    buttons.appendChild(okBtn); buttons.appendChild(cancelBtn2);
                    dialog.appendChild(title); dialog.appendChild(tips); dialog.appendChild(targetContainer); dialog.appendChild(buttons);
                ...
                saveButton.onclick = async () => {
                    if (zmlLoraContentEditCurrentLoraPath) {
                        try {
                            // === 智能保存策略：先探测可读性 ===
                            const probeRes = await api.fetchApi(`${ZML_API_PREFIX}/probe_save_targets`, {
                                method: "POST",
                                headers: { "Content-Type": "application/json" },
                                body: JSON.stringify({
                            ...
                            // 解析探测结果
                            const zmlDirExists = !!probe.zml_dir_exists;
                            const zmlReadable = !!probe.zml_has_readable;
                            const sameReadable = !!probe.same_has_readable;

                            // 依据六种情形决定保存目标：
                            // 1) 存在zml文件夹（且可读取），无同层级可读 -> 全部保存到 zml
                            // 3) 存在zml文件夹（但不可读取），无同层级可读 -> 弹窗选择保存位置
                            // 4) 不存在zml文件夹，有同层级可读 -> 全部保存到 same
                            // 5) 不存在zml文件夹，且无同层级可读 -> 弹窗选择保存位置
                            // 6) 同时存在可读 zml 与同层级 -> 同时双写两个位置
                            // 2)（未编号但推断）存在zml可读且同层级可读 -> 属于6)

                            let targets = [];
                            let needChoice = false;

                            if (zmlDirExists && zmlReadable && !sameReadable) {
                                targets = ["zml"]; // 情形1
                            } else if (zmlDirExists && !zmlReadable && !sameReadable) {
                                needChoice = true; // 情形3
                            } else if (!zmlDirExists && sameReadable) {
                                targets = ["same"]; // 情形4
                            } else if (!zmlDirExists && !sameReadable) {
                                needChoice = true; // 情形5
                            } else if (zmlReadable && sameReadable) {
                                targets = ["zml", "same"]; // 情形6
                            } else if (zmlDirExists && zmlReadable && sameReadable === false) {
                                targets = ["zml"]; // 覆盖边界
                            } else if (zmlDirExists && sameReadable && !zmlReadable) {
                                // zml目录存在但不可读，同层级可读 -> 默认 same，用户也许希望写zml，保持同层级以不破坏现状
                                targets = ["same"]; 
                            }
                            ...
                            if (needChoice || targets.length === 0) {
                                ...
                                // userTarget: "zml" | "same" | "both" | null(cancel)
                                if (!userTarget) {
                                    hideLoraContentEditModal();
                                    return;
                                }
                                if (userTarget === "both") targets = ["zml", "same"]; else targets = [userTarget];
                            }

                            // 执行保存（可能双写）
                            const saveOne = async (file_type, content, target) => {
                                const res = await api.fetchApi(`${ZML_API_PREFIX}/save_lora_file`, {
                                    method: "POST",
                                    headers: { "Content-Type": "application/json" },
                                    body: JSON.stringify({
                                        "lora_filename": zmlLoraContentEditCurrentLoraPath,
                            ...
                            for (const t of targets) {
                                const r1 = await saveOne("txt", zmlLoraContentEditTxtTextarea.value, t);
                                const r2 = await saveOne("json", zmlLoraContentEditJsonTextarea.value, t);
                                const r3 = await saveOne("log", zmlLoraContentEditLogTextarea.value, t);
                                results.push(r1, r2, r3);
                            }

                            const allOk = results.every(r => r && r.status === "success");
                            if (allOk) {
                                const targetText = targets.length === 2 ? "zml 和 同层级" : (targets[0] === "zml" ? "zml" : "同层级");
                                alert(`LoRA '${zmlLoraContentEditCurrentLoraName}' 的内容已保存到 ${targetText}！`);
                            } else {
                ...
                filesToDisplay.forEach(file => {
                    ...
                    fetchMetadataBtn.onclick = async (e) => {
                        ...
                        if (isNoPreviewAndFetch) {
                            ...
                            // 选择保存位置（同层级 / zml子目录）
                            const targetContainer = zmlCreateEl("div", {
                                style: `
                                    margin: 8px 0 16px 0;
                                    display: flex;
                                    gap: 16px;
                            ...
                            const targetLabel = zmlCreateEl("span", { style: `color: #ccc; font-size: 13px;`, textContent: "保存位置" });
                            const sameLabel = zmlCreateEl("label", { style: `display: flex; gap: 6px; align-items: center; cursor: pointer;` });
                            const inputSame = document.createElement("input"); inputSame.type = "radio"; inputSame.name = "zml-save-target"; inputSame.value = "same"; inputSame.checked = true;
                            const sameText = zmlCreateEl("span", { textContent: "同层级" });
                            sameLabel.appendChild(inputSame); sameLabel.appendChild(sameText);
                            const zmlLabel = zmlCreateEl("label", { style: `display: flex; gap: 6px; align-items: center; cursor: pointer;` });
                            const inputZml = document.createElement("input"); inputZml.type = "radio"; inputZml.name = "zml-save-target"; inputZml.value = "zml";
                            const zmlText = zmlCreateEl("span", { textContent: "zml子目录" });
                            zmlLabel.appendChild(inputZml); zmlLabel.appendChild(zmlText);
                            targetContainer.appendChild(targetLabel);
                            targetContainer.appendChild(sameLabel);
                            targetContainer.appendChild(zmlLabel);
                            ...
                            try {
                                    const target = inputZml.checked ? "zml" : "same";
                                    const response = await api.fetchApi(`${ZML_API_PREFIX}/fetch_civitai_metadata`, {
                                        method: "POST",
                                        headers: { "Content-Type": "application/json" },
                                        body: JSON.stringify({ "lora_filename": loraPath, "target": target }),
                                    });
                                    const result = await response.json();
                            ...
                            confirmDialog.appendChild(targetContainer);
                 ...
                 try {
                     ...
                     this.createLoraEntryDOM = (entry) => { // Removed index parameter as it's not strictly needed for rendering
                         ...
                         const entryCard = zmlCreateEl("div", { // <-- 这里会调用到局部定义的 zmlCreateEl
                             ...
                             style: `display: flex; align-items: center; gap: 4px; padding: ${s.cardPadding}; background: ${entry.enabled ? this.loraEntryColor : adjustBrightness(this.loraEntryColor, -10, -15)}; border-radius: 2px; ${entry.enabled && this.isLocked ? `border: 3px solid ${this.enabledStateColor};` : 'border: 1px solid transparent;'} cursor: pointer;` // 添加边框样式和光标
                         });
                         ...
                         entryCard.onclick = (e) => {
                             ...
                             if (e.target.tagName === 'INPUT' || e.target.tagName === 'BUTTON' || e.target.tagName === 'TEXTAREA' || 
                                 e.target.classList.contains('zml-pll-drag-handle') || 
                                 e.target.classList.contains('zml-lora-entry-edit') ||
                                 e.target.classList.contains('zml-weight-btn') ||
                                 e.target.parentElement?.classList.contains('zml-lora-entry-delete')) {
                                 return;
                             }
                         ...
                         const displayNameInput = zmlCreateEl("input", { className: "zml-lora-display-name-input", type: "text", value: entry.display_name, placeholder: "输入名称...", title: "自定义此LoRA条目的显示名称", style: `width: ${this.loraNameWidth}px;` });// <-- 这里会调用到局部定义的 zmlCreateEl
                         ...
                         const loraSelectorBtn = zmlCreateEl("button", { style: `flex-grow: 1; min-width: 100px; padding: ${s.inputPadding}; background: #222; border: 1px solid #555; border-radius: 2px; color: #ccc; text-align: left; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; height: ${s.inputHeight};`, textContent: entry.lora_name === "None" ? "None" : (entry.lora_name || "").split(/[/\\]/).pop() });// <-- 这里会调用到局部定义的 zmlCreateEl
                         ...
                         editBtn.onclick = async (e) => {
                             ...
                             if (entry.lora_name && entry.lora_name !== "None") {
                                 ...
                                 if (hasPreview) {
                                     ...
                                     const targetLabel = zmlCreateEl("span", { style: `color: #ccc; font-size: 13px;`, textContent: "保存位置" });
                                     const sameLabel = zmlCreateEl("label", { style: `display: flex; gap: 6px; align-items: center; cursor: pointer;` });
                                     const inputSame = document.createElement("input"); inputSame.type = "radio"; inputSame.name = "zml-save-target-single"; inputSame.value = "same"; inputSame.checked = true;
                                     const sameText = zmlCreateEl("span", { textContent: "同层级" });
                                     sameLabel.appendChild(inputSame); sameLabel.appendChild(sameText);
                                     const zmlLabel = zmlCreateEl("label", { style: `display: flex; gap: 6px; align-items: center; cursor: pointer;` });
                                     const inputZml = document.createElement("input"); inputZml.type = "radio"; inputZml.name = "zml-save-target-single"; inputZml.value = "zml";
                                     const zmlText = zmlCreateEl("span", { textContent: "zml子目录" });
                                     zmlLabel.appendChild(inputZml); zmlLabel.appendChild(zmlText);
                                     targetContainer.appendChild(targetLabel);
                                     targetContainer.appendChild(sameLabel);
                     ...
                     this.createFolderDOM = (entry) => { // Removed index parameter
                         ...
                         savePresetBtn.onclick = (e) => {
                             ...
                             try {
                                 // 调用保存预设API端点（与后端路由保持一致）
                                 api.fetchApi(`${ZML_API_PREFIX}/save_lora_preset`, {
                                     method: "POST",
                                     headers: { "Content-Type": "application/json" },
                         ...
                         header.addEventListener("mousedown", (e) => {
                             e.stopImmediatePropagation(); // 阻止所有其他事件监听器
                             // 先检查目标元素是否是输入框、删除按钮、保存预设按钮或拖动句柄
                             if (e.target === nameInput || e.target === deleteBtn || e.target === savePresetBtn || e.target === dragHandle) {
                                 return; // 不阻止这些元素的默认行为
                             }
                             e.preventDefault(); // 阻止默认的 mousedown 行为
            ...
            // === 预设管理功能 ===
            // 加载预设面板函数
            nodeType.prototype.loadPresetPanel = function() {
                // 检查是否已存在预设面板
                let overlay = document.getElementById('zml-preset-overlay');
                let presetPanel = document.getElementById('zml-preset-panel');
                
                if (overlay && presetPanel) {
                    // 如果已存在，直接显示
                ...
                overlay = zmlCreateEl("div", {
                    id: "zml-preset-overlay",
                    style: "position: fixed; top: 0; left: 0; width: 100%; height: 100%; background: rgba(0, 0, 0, 0.5); z-index: 999; display: block;"
                });
                
                // 创建预设面板
                presetPanel = zmlCreateEl("div", {
                    id: "zml-preset-panel",
                    style: "position: fixed; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 600px; height: 500px; background: #333; border: 1px solid #555; border-radius: 8px; z-index: 1000; display: flex; flex-direction: column; box-shadow: 0 4px 20px rgba(0,0,0,0.5);"
                });
                ...
                const panelHeader = zmlCreateEl("div", {
                    style: "background: #444; padding: 10px 15px; display: flex; justify-content: space-between; align-items: center; cursor: move; border-radius: 8px 8px 0 0; border-bottom: 1px solid #555;"
                });
                ...
                panelHeader.onmousedown = (e) => {
                    ...
                    presetPanel.style.left = panelStartX + 'px';
                    presetPanel.style.top = panelStartY + 'px';
                };
                ...
                const handleDragMove = (e) => {
                    ...
                    // 限制在视口内
                    const maxX = window.innerWidth - presetPanel.offsetWidth;
                    const maxY = window.innerHeight - presetPanel.offsetHeight;
                    
                    newX = Math.max(0, Math.min(maxX, newX));
                    newY = Math.max(0, Math.min(maxY, newY));
                    
                    presetPanel.style.left = newX + 'px';
                    presetPanel.style.top = newY + 'px';
                };
                ...
                const resizeHandle = zmlCreateEl("div", {
                    style: "position: absolute; bottom: 0; right: 0; width: 20px; height: 20px; cursor: se-resize; background: linear-gradient(-45deg, transparent 0%, transparent 40%, #666 40%, #666 60%, transparent 60%); z-index: 1001;"
                });
            ...
            // 加载预设列表函数
            nodeType.prototype.loadPresets = function() {
                const contentArea = document.getElementById('zml-preset-content');
                ...
                // 尝试从API获取预设（使用原版的获取端点）
                api.fetchApi(`${ZML_API_PREFIX}/get_lora_presets`, {
                    method: "GET"
                })
                .then(response => response.json())
                .then(data => {
                    ...
                    if (data.status === "success" && data.presets && data.presets.length > 0) {
                        ...
                        data.presets.forEach(preset => {
                            this.createPresetFolder(preset);
                        });
                    } else {
            ...
            // 创建预设文件夹函数
            nodeType.prototype.createPresetFolder = function(preset) {
                const contentArea = document.getElementById('zml-preset-content');
                if (!contentArea) return;
            ...
            // 应用预设函数
            nodeType.prototype.applyPreset = function(preset) {
            ...
            // 关闭面板函数
            nodeType.prototype.closePresetPanel = function() {
                const overlay = document.getElementById('zml-preset-overlay');
                const presetPanel = document.getElementById('zml-preset-panel');
            ...
            // 删除预设函数
            nodeType.prototype.deletePreset = function(presetName) {
                if (!presetName) {
                    alert("预设名称无效，无法删除！");
                    return;
                }
                ...
                // 调用删除API（与后端路由保持一致）
                api.fetchApi(`${ZML_API_PREFIX}/delete_lora_preset`, {
                    method: "POST",
                    headers: {
