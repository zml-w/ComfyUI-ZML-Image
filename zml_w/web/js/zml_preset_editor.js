    ...
    dialog.innerHTML = `
        <div class="zml-dialog-content">
            ...
            <div class="zml-dialog-footer">
                <span class="zml-dialog-notice">可以通过comfyui左上角编辑（工作流按钮旁边）里的‘刷新节点定义’来更新节点</span>
 ...
 * @param {object} node - LiteGraph节点实例
 */
function createResolutionDialog(node) {
    // 如果已存在一个弹窗，则先移除
    const existingDialog = document.querySelector(".zml-preset-dialog");
    if (existingDialog) {
        existingDialog.remove();
    ...
    dialog.innerHTML = `
        <div class="zml-dialog-content">
            <h2>添加新的分辨率预设</h2>
            <div class="zml-form-row">
                <label for="zml-preset-width">宽:</label>
                <input type="number" id="zml-preset-width" name="zml-preset-width" value="1024">
            </div>
            <div class="zml-form-row">
                <label for="zml-preset-height">高:</label>
                <input type="number" id="zml-preset-height" name="zml-preset-height" value="768">
            </div>
            <div class="zml-dialog-footer">
                <span class="zml-dialog-notice">可以通过comfyui左上角编辑（工作流按钮旁边）里的‘刷新节点定义’来更新节点</span>
                <div class="zml-dialog-buttons">
    ...
    const saveButton = dialog.querySelector("#zml-save-preset");
    const cancelButton = dialog.querySelector("#zml-cancel-preset");
    const widthInput = dialog.querySelector("#zml-preset-width");
    const heightInput = dialog.querySelector("#zml-preset-height");
...
app.registerExtension({
    ...
    nodeCreated(node) {
        ...
        if (node.comfyClass === "ZML_PresetResolution") {
            node.addWidget( "button", "添加预设", null, () => createResolutionDialog(node) );
        }
    }
});