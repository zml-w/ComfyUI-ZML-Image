...
try:
    ...
    print("ZML_CropPureColorBackground: scipy not found. '不规则形状保留像素' feature will be disabled.")
    ...
    def add_watermark(self, 图像, 文本, 字体, 字体大小, 颜色, 不透明度, 书写方向, 位置, 水平边距, 垂直边距, 字符间距, 行间距, 全屏水印旋转角度, 全屏水印密度):
        ...
        for img_tensor in 图像:
            ...
            if 位置 == "全屏":
                ...
                padding, tile_w, tile_h = 10, tw + (2*padding), th + (2*padding)
                text_img, text_draw = Image.new('RGBA', (tile_w, tile_h)), ImageDraw.Draw(text_img); self._draw_text_manually(text_draw, lines, padding, padding, font, fill_color, 不透明度, 字符间距, 行间距, 书写方向)
